...
# 生成公钥密钥
def set_up(security_parameter: int, vector_length: int, G) -> Tuple[List[ElGamalKey], List[ElGamalKey]]:
    master_public_key = [None] * vector_length
    master_secret_key = [None] * vector_length
    for i in range(vector_length):
        (master_public_key[i], master_secret_key[i]) = G.keygen(secparam=security_parameter)
...
def KeyDerive(master_secret_key: List[ElGamalKey], master_public_key2: List[ElGamalKey],
    ...
    for i in range(len(y)):
        ...
        # print(key)
        r = H1(f"{key}{ctr}{aux}",
               elgamal_params['p'])  # pk2i的'g'分量的master_secret_key次方，pk2i的'x'分量，计数器ctr，辅助字符串aux，以及模数p。
        skf += r * y[i]
    print(type(skf))
...
def Encrypt(master_public_key: List[ElGamalKey], master_secret_key2: List[ElGamalKey],
            master_public_key3: List[ElGamalKey], ctr: int, X: List[int], aux: str) -> integer:
    C = []
    ctr += 1
...
def Decrypt(master_public_key: List[ElGamalKey], skf: List[ElGamalKey], master_secret_key3: List[ElGamalKey],
            C: List[ElGamalKey], y: List[int]) -> integer:
